...
public class ZoomImage extends ImageView implements ScaleGestureDetector.OnScaleGestureListener,
    ...
    private OnClickListener pointerUpListener;
    // 解决滑动过程中将事件交付给ViewPager后无法再次获取的问题
    ...
    public ZoomImage(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.cl_ZoomImageAttr);
        mPlaceHolderId = typedArray.getResourceId(R.styleable.cl_ZoomImageAttr_cl_placeHolderId, 0);
        mErrorHolderId = typedArray.getResourceId(R.styleable.cl_ZoomImageAttr_cl_errorHolderId, 0);
        typedArray.recycle();
        if (mPlaceHolderId != 0) {
            setImageResource(mPlaceHolderId);
        }
        ...
        mGestureDetector = new GestureDetector(context,new GestureDetector.SimpleOnGestureListener(){
            ...
            public boolean onSingleTapUp(MotionEvent e) {
                ...
                if (listener != null) {
                    listener.onClick(ZoomImage.this);
                }
    ...
    public void onGlobalLayout() {
        ...
        if (!mFirst){
            ...
            if (d == null && mErrorHolderId != 0) {
                d = getResources().getDrawable(mErrorHolderId);
                setImageDrawable(d);
            }
...
//                }
    ...
    /**
     * 自动放大缩小，自动缩放的原理是使用View.postDelay()方法，每隔8ms调用一次
    ...
    public boolean onScale(ScaleGestureDetector detector) {
        ...
        if ((scaleFactor > 1.0f && scale * scaleFactor < mMaxOverScale)
            ...
            if (pointerUpListener != null) {
                pointerUpListener.onClick(this);
            }
        }
        return true;
    }
                ...
                if (Math.abs(dx) > 5 || Math.abs(dy) > 5) {
                    ...
                    if (rectF.width() > getWidth() + 0.01f || rectF.height() > getHeight() + 0.01f) {
                        ...
                        if (intercepted) {
                            ...
                            if ((dx > 0 && Math.round(rectF.left) != 0) || (dx < 0 && Math.round(rectF.right) != getRight())) {
                                intercepted = false;
                                if (parent != null && parent instanceof ViewPager) {
                                    getParent().requestDisallowInterceptTouchEvent(true);
                                }
    ...
    public void setOnPointerUpListener(OnClickListener listener) {
